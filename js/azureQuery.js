...
azureQueryStorage.prototype = function() {
			...
			success : function(data, status, xhr) {
				if (data) {
					for (var c = 0; c < data.length; c++) {
						data[c].create = function() {
							createContainer(this.name);
						};
						data[c].remove = function() {
							removeContainer(this.name);
						};
					}
				}
	...
	// iterates through containers or blobs.
		...
		if (this.focus === "containers") {
			...
			container = {
				name : self.currentContainerName
			};
			func(container);
	...
	// remove current seletion.
		...
		if (!aq.storageOptions.allowDeletes)
			throw ('Cannot delete - delete permission has not been set');
	...
	// text() ............... get blob as text
		...
		if (arguments.length == 0)/* get */
		{
			...
			if (this.focus === "blobs") {
				if (!this.blobCollection || this.blobCollection.length === 0)
					return this;
